    ...
    useEffect(() => {
        const getPredictions = async () => {
            try {
                ...
                const betzonaLinks = state.predictionsSlice.betzona.filter(
                    (el) =>
                        findTeam(el.homeName, props.homeName) &&
                        findTeam(el.awayName, props.awayName)
                );
                let betzona;
                if (betzonaLinks.length !== 0) {
                    const betzonaPredict =
                        await predictionsServices.getBetzonaPredict(
                            betzonaLinks[0].link
                        );
                ...
                const oddsRuLinks = state.predictionsSlice.oddsRu.filter(
                    (el) =>
                        findTeam(el.homeName, props.homeName) &&
                        findTeam(el.awayName, props.awayName)
                );
                ...
                if (oddsRuLinks.length !== 0) {
                    const oddsPredict =
                        await predictionsServices.getOddsRuPredict(
                            oddsRuLinks[0].link
                        );
                ...
                const legalbetlLink = state.predictionsSlice.legalbet.filter(
                    (el) =>
                        findTeam(el.homeName, props.homeName) &&
                        findTeam(el.awayName, props.awayName)
                );
                ...
                if (legalbetlLink.length !== 0) {
                    const legalbetPredict =
                        await predictionsServices.getLeagalbetPredict(
                            legalbetlLink[0].link
                        );
                ...
                const liveresultFilterOnNull =
                    state.predictionsSlice.liveresult.filter(
                        (el) => el.homeName && el.awayName
                    );
                const liveresultlLink = liveresultFilterOnNull.filter(
                    (el) =>
                        findTeam(el.homeName, props.homeName) &&
                        findTeam(el.awayName, props.awayName)
                );
                ...
                if (liveresultlLink.length !== 0) {
                    const liveresultPredict =
                        await predictionsServices.getLiveresultPredict(
                            liveresultlLink[0].link
                        );
                ...
                const arrPredictions = [
                    ...betzona,
                    ...oddsRu,
                    ...legalbet,
                    ...liveresult,
                ];
                const arrPredictionsFilter = arrPredictions.filter(
                    (el) => el.predict !== "" && el.text !== ""
                );
