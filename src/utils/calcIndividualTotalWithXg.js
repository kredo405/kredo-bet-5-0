...
export const calcIndividualTotalWithXg = async (data) => {
    ...
    const arrMiddlePeasant = data.table[0].filter(
        (el) => el["10"] - el["11"] <= 6 && el["10"] - el["11"] > -6
    );
    const arrOutsiders = data.table[0].filter(
        (el) => el["10"] - el["11"] <= -6
    );
    ...
    const calcScore = (matches, teamName) => {
        ...
        matches.forEach((el) => {
            if (teamName === el.team1_name) {
                const resGrands = arrGrands.find(
                    (item) => item["2"] === el.team2_name
                );
                const resMidle = arrMiddlePeasant.find(
                    (item) => item["2"] === el.team2_name
                );
                const resOutsider = arrOutsiders.find(
                    (item) => item["2"] === el.team2_name
                );
    ...
    const scoreAwayVs =
        (calcScoreVs(matchesAwayFilter, data.team2_name) + 1) / 6;
    ...
    const getDataLastMatches = async (matches) => {
        const arrMatches = await Promise.all(
    ...
    if (
        ...
        resAway.xg > 0 &&
        resHome.xgVs > 0 &&
        expectedGoalsHome >= 0.3 &&
        expectedGoalsAway >= 0.3
    ) {
        individualTotalHome =
            ((resHome.xg + resAway.xgVs) / 2 + expectedGoalsHome) / 2;
        individualTotalAway =
            ((resAway.xg + resHome.xgVs) / 2 + expectedGoalsAway) / 2;
